...
func newTestCache(t *testing.T, size int64, recordSize int) (*RingBufferCache, string) {
    t.Helper()
    dir := t.TempDir()
    base := filepath.Join(dir, "cache.data")
    opts := DefaultOptions()
    // Make tests portable across OS / CI environments by disabling mmap and
    ...
    c, err := NewRingBufferCacheWithOptions(base, size, recordSize, opts)
    if err != nil {
...
func TestFlushClosePersistence(t *testing.T) {
    ...
    opts.UseMmap = false
    opts.ShardCount = 1
    reopened, err := NewRingBufferCacheWithOptions(base, 5, 12, opts)
    if err != nil {
